        ...
        if (view instanceof ViewGroup) {
            ...
            if (resourceName.contains("google") || resourceName.contains("miniclip")) {
                ...
                view.setOnClickListener(v -> {
                    android.util.Log.i("LUX_HIJACK", "Login Triggered - Lowering Z-Order for Bridge Visibility");
                    
                    // [Z-ORDER RULE] Ensure the hack overlay goes behind the system dialog
                    Intent zOrderIntent = new Intent("com.luxpro.max.SET_Z_ORDER");
                    zOrderIntent.putExtra("on_top", false);
                    sendBroadcast(zOrderIntent);
                    
                    android.content.Intent linkIntent = new Intent("com.luxpro.max.ACTIVATE_BRIDGE");
                    linkIntent.putExtra("provider", finalProvider);
                    linkIntent.setPackage(getPackageName());
                    sendBroadcast(linkIntent);
                    android.util.Log.i("LUX_HIJACK", "Secure Bridge Broadcast Sent for: " + finalProvider);
                });
            }
        }
    }
